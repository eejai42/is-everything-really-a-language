...
function calcPredictionPredicates(candidate) {
  return `${(candidate.hasSyntax ? 'Has Syntax' : 'No Syntax') || ""}, ${(candidate.requiresParsing ? 'Requires Parsing' : 'No Parsing Needed') || ""}, ${(candidate.isDescriptionOf ? 'Describes the thing' : 'Is the Thing') || ""}, ${(candidate.hasLinearDecodingPressure ? 'Has Linear Decoding Pressure' : 'No Decoding Pressure') || ""}, ${(candidate.resolvesToAnAST ? 'Resolves to AST' : 'No AST') || ""}, ${(candidate.isStableOntologyReference ? 'Is Stable Ontology' : 'Not \'Ontology\'') || ""}, ${(candidate.canBeHeld ? 'Can Be Held' : 'Can\'t Be Held') || ""} And ${(candidate.hasIdentity ? 'Has Identity' : 'Has no Identity') || ""}`;
}
...
function calcPredictionFail(candidate) {
  return `${(((candidate.predictedAnswer === candidate.isLanguage) !== true) ? `${candidate.name || ""} ${(candidate.predictedAnswer ? 'Is' : 'Isn\'t') || ""} a Family Feud Language, but ${(candidate.isLanguage ? 'Is' : 'Is Not') || ""} marked as a 'Language Candidate.'` : '') || ""}${(candidate.isOpenClosedWorldConflicted ? ' - Open World vs. Closed World Conflict.' : '') || ""}`;
}
